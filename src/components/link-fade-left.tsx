...
export const LinkFadeLeft = ({ children, url, className }: IProps) => {
  const exitTransition = {
    ...
    exitTrigger: useCallback((container: any) => {
        ...
        'style',
        'animation: fadeLeftOut 0.8s cubic-bezier(.25,.1,.25,1) forwards;'
      );
    }, []),
  };
  ...
  const entryTransition = {
    ...
    entryTrigger: useCallback((container: any) => {
      container.childNodes[0].childNodes[1].setAttribute(
        'style',
        'animation: fadeLeftIn 0.8s cubic-bezier(.25,.1,.25,1) forwards;'
      );
    }, []),
  };
